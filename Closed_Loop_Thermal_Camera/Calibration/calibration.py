...
label_hot = label.Label(terminalio.FONT, text="40C", color=0xFFFFFF,
                        x=BAR_X - 20, y=BAR_Y + 4)
label_cold = label.Label(terminalio.FONT, text="20C", color=0xFFFFFF,
...
# ==============================
# Drawing Helpers (HORIZONTAL FLIP ONLY)
# ==============================
def fx(x):
    return WIDTH - 1 - x
...
def draw_cross(bitmap, px, py, color=63):
    for dx in (-1, 0, 1):
        if 0 <= px + dx < WIDTH:
            bitmap[fx(px + dx), py] = color
    for dy in (-1, 0, 1):
        if 0 <= py + dy < HEIGHT:
            bitmap[fx(px), py + dy] = color

def draw_block(bitmap, gx, gy, color=40):
    x0 = gx * PELTIER_W
    y0 = gy * PELTIER_H
    for x in range(x0, min(x0 + PELTIER_W, WIDTH)):
        bitmap[fx(x), y0] = color
        bitmap[fx(x), min(y0 + PELTIER_H - 1, HEIGHT - 1)] = color
    for y in range(y0, min(y0 + PELTIER_H, HEIGHT)):
        bitmap[fx(x0), y] = color
        bitmap[fx(min(x0 + PELTIER_W - 1, WIDTH - 1)), y] = color
...
while True:
    ...
    # Render image (horizontal flip only)
    ...
    if max_temp >= THRESHOLD:
        draw_cross(bitmap, px, py)
        draw_block(bitmap, gx, gy)
        hot_label.text = f"HOT px({px},{py}) grid({gx},{gy}) {max_temp:.1f}C"
